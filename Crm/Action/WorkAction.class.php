...
class WorkAction extends CommonAction {
    ...
    public function addCustomer(){
        ...
        if (IS_POST) {
            ...
            if (M('work')->where(array('Userid' => array('IN', $user)))->where("Tel =".I('post.Tel'))->select()) {
                $this->error('该客户已存在', __SELF__, 1);
            }

            // 自动验证
            if (!$data) {
                $this->error(D('Work')->getError(), __SELF__, 1);
            }
            ...
            $id = D('Work')->relation(true)->add($data);
            if ($id) {
                // 添加操作记录 以便及时提醒相关人员
                $this->addnews($data['Designer'], $data['Userid'], $data['Project'], $id, 'Work/visitRecord', '添加了一条客户信息');
    ...
    public function updateCustomer(){
        ...
        if (IS_POST) {
            ...
            // 处理标记颜色
            $oldcolor = M('work_attached')->where(array('customer_id' => $data['id']))->getField('markcolor');
            ...
            // 更新客户信息
            if (D('Work')->relation(true)->save($data) !== false) {
                // 添加操作记录 以便及时提醒相关人员
               // $this->addnews($data['Designer'], $data['Userid'], $data['Project'], $data['id'], 'Work/visitRecord', '更新了一条客户信息');
            ...
            // 查询当前客户信息
            $info = D('WorkView')->where(array('id' => $this->_get('id')))->find();
            // 查询工程组属性的部门
            $where = array(
                'admin_id' => fid(),
                'class' => 5
    ...
    public function deleteCustomer(){
        ...
        // 删除客户
        if (D('Work')->relation(true)->save($data)) {
    ...
    public function deleteForever(){
        ...
        $cid = M('work_platform')->where(array('customer_id' => $data['id']))->find();
        if ($cid) {
            $this->error('该客户已开通客户平台,不能删除!!');
        }

        // 更新数据
        if (M('work')->where($data)->delete()) {
    ...
    public function visitRecord(){
        ...
        if (IS_POST) {
            ...
            // 查询是否存在该客户
            $c = M('work')->field('id')->where(array('id' => $data['customer_id']))->find();
            if (!$c) {
                $this->error('不存在该客户', __SELF__, 1);
            }

            // 添加记录
            if (M('work_record')->add($data)) {
                // 添加操作记录 以便及时提醒相关人员
                $this->addnews($c['Designer'], $c['Userid'], $c['Project'], $c['id'], 'Work/visitRecord', '添加了一条回访信息');
            ...
            $pre = M('work')->where($data)->order('id desc')->getField('id');
            $next = M('work')->where($data2)->order('id asc')->getField('id');

            // 追踪记录
            $record = M('work_record')->where(array('customer_id' => $this->_get('id')))->select();
    ...
    public function remindvisitRecord(){
        ...
        if (IS_POST) {
            ...
            // 查询是否存在该客户
            $c = M('work')->field('id,Designer,Userid,Project')->where(array('id' => $data['customer_id']))->find();
            if (!$c) {
                $this->error('不存在该客户', __SELF__, 1);
            }
            ...
            $pre = M('work')->where($data)->where($map)->order('id desc')->getField('id');
            $next = M('work')->where($data2)->where($map)->order('id asc')->getField('id');
    ...
    public function updateRecord(){
        ...
        if (IS_POST) {
            ...
            // 修改记录
            if (M('record')->save($data) || M('work')->save($map)) {
    ...
    public function search(){
        ...
        // 判断搜索条件
        if (!empty($Company)) {
            $map['Company'] = $Company; // 项目名称
        }
        if (!empty($Contact)) {
            $map['Contact'] = $Contact; // 项目名称
        }
        if (!empty($Tel)) {
            $map['Tel'] = $Tel; // 联系方式
        }
        if (!empty($Userid)) {
            $map['Userid'] = $Userid; // 业务员id
        }
        if (!empty($Designer)) {
            $map['Designer'] = $Designer; // 设计师id
        }
        if (!empty($Project)) {
            $map['Project'] = $Project; // 工程监理id
        }
        if (!empty($hetongbianhao)) {
            $map['hetongbianhao'] = $hetongbianhao; // 合同编号
        }
        ...
        // 调用公共分页
        $this->pageCommon($map, $sort, $status);
        $sum  = D('WorkView')->where($map)->Sum('OrdersValue');  // 符合查询条件的总单值
    ...
    public function displayFields(){
        ...
        //提取POST数据
        $data['uid'] = session('uid');
        $data['work_field'] = $this->_post('tijiao');
    ...
    public function projectFields(){
        ...
        //提取POST数据
        $data['uid'] = session('uid');
        $data['work_project_field'] = $this->_post('tijiao');
    ...
    protected function attachedInfo(){
        ...
        // 自定义显示字段
        $fields = M('user_defined')->field('work_field')->where(array('uid' => session('uid')))->find();
        session('work_field', $fields);
...
//            return $where = array();
    ...
    public function pageCommon($map, $sort='null', $status=0){
        ...
        $count  = D('WorkView')->where($map)->count();  // 查询记录总数
        ...
        if (!empty($sort)) {
            if ($status) {
                $list = D('WorkView')->where($map)->limit($Page->firstRow.','.$Page->listRows)->order("$sort desc,id desc")->select();
            } else {
                $list = D('WorkView')->where($map)->limit($Page->firstRow.','.$Page->listRows)->order("$sort asc,id desc")->select();
            }
            $this->assign('status', $status);
        } else {
            $list = D('WorkView')->where($map)->limit($Page->firstRow.','.$Page->listRows)->order('id desc')->select();
        }
        ...
        // 符合查询条件的总单值
        $sum1  = D('WorkView')->where($map)->Sum('OrdersValue');
        $dingjin  = D('WorkView')->where($map)->Sum('Deposit');
        $space  = D('WorkView')->where($map)->Sum('Space');
        $shejifei  = D('WorkView')->where($map)->Sum('shejifei');
        $guanlifei  = D('WorkView')->where($map)->Sum('guanlifei');
        $qingfu  = D('WorkView')->where($map)->Sum('qingfu');
        $zhucai  = D('WorkView')->where($map)->Sum('zhucai');
        $once  = D('WorkView')->where($map)->Sum('once');
        $twice  = D('WorkView')->where($map)->Sum('twice');
        $tirth  = D('WorkView')->where($map)->Sum('tirth');
        $others  = D('WorkView')->where($map)->Sum('others');
