...
Class ProjectAction extends CommonAction {
    ...
    public function openPaltform() {
        ...
        $cid = M('customer_platform')->where($where)->order('customer_id asc')->getField('customer_id', true);
    ...
    public function chooseState(){
        if(IS_POST){
            ...
            // 获取当前用户选中的已选中的客户状态
            $stated = M('user_defined')->where(array('uid' => session('uid')))->getField('pstate');
    ...
    public function search(){
        ...
        $CName = I('get.CName'); // 客户姓名
        $Tel = I('get.Tel'); // 联系方式
        $Address = I('get.Address'); // 所在小区
        $Userid = I('get.Userid', 0, 'intval');  // 业务员id
        $Designer = I('get.Designer', 0, 'intval');  // 设计师id
        $Project = I('get.Project', 0, 'intval');    // 工程监理id
        $Captain = I('get.Captain', 0, 'intval');    // 工长id
        $Number = I('get.Number');    // 合同编号

        // 判断是否存在状态值 若不存在 则直接在施工 保修 两个状态中搜索 若存在 则在住对应状态下搜索
/*        if (!empty($state)) {
            $map['_string'] = "POSITION($state IN CONCAT(',',State,','))";
        } else {
            // 状态为施工阶段-12 和 保修阶段-13 的客户
            $where['admin_id'] = fid();
            $where['state_id'] = array('IN', array('12','13'));
            $state = M('com_state')->where($where)->getField('id', true);
            $map['_string'] = "POSITION($state[0] IN CONCAT(',',State,',')) OR POSITION($state[1] IN CONCAT(',',State,','))";
        }*/

        // 判断搜索条件
        if (!empty($CName)) {
            $map['CName'] = I('get.CName'); // 客户姓名
        }
        if (!empty($Tel)) {
            $map['Tel']= I('get.Tel'); // 联系方式
        }
        if (!empty($Address)) {
            $map['Address'] = I('get.Address'); // 所在小区
        ...
        if (!empty($Captain)) {
            $map['Captain'] = I('get.Captain', 0, 'intval');    // 工长id
        }
        if (!empty($Number)) {
            $map['Number'] = I('get.Number'); // 合同编号
        }
    ...
    public function addPlatform(){
        ...
        if (IS_POST) {
            // 验证只能输入汉字和字符
            $validate = array(
                array('cusname', '/^[a-zA-Z][\w]{4,16}$/','用户名需以字母开头，5-17个字符 字母、数字、下划线_'),
                array('password', '/^[a-zA-Z][\w]{4,16}$/','密码需以字母开头，5-17个字符 字母、数字、下划线_'),
                array('cusname', '', '该用户名已经存在!', 0, 'unique', 1),
                array('customer_id', '', '该客户已经开通平台!', 0, 'unique', 1),
            );
            D('customer_platform')->setProperty("_validate",$validate);

            // 创建数据集
            $data = D('customer_platform')->create();
            if (!$data) {
                $this->error(D('customer_platform')->getError());
            }
            ...
            if (D('customer_platform')->add($data)) {
                deduct_add_platform();
            ...
            // 客户信息
            $info = D('CustomerView')->where(array('id' => I('get.id', 0, 'intval')))->find();

            // 查询条件
            $where['customer_id'] =  $info['id'];

            // 判断是否开通客户平台   开通的话则返回平台状态 0-开启 1-关闭  没开通返回 2
            $status = M('customer_platform')->where($where)->getField('open');
            ...
            // 获取客户平台的用户名
            $user = M('customer_platform')->field('cusname,password,project')->where($where)->find();

            // 获取所有二级施工工序
            $parr = unserialize($user['project']);  //反序列化成数组
            $list = M('project')->where(array('id' => array('IN', $parr)))->order('sort asc')->select();
            $list = node_merge($list);

            // 获取当前客户的所有施工详情
            $reslut = D('PlatinfoView')->where($where)->order('sort desc')->select();
            ...
            //获取客户咨询问题
            $comments = M('customer_comment')->where(array('customer_id'=>$info['id']))->select();
    ...
    public function selectPlatform(){
        ...
        if (IS_POST) {
            ...
            $data['project'] = serialize(I('post.project'));  //序列化

            // 判断客户是否开通客户平台
            $isopen = M('customer_platform')->where($where)->getField('id');
            if (empty($isopen)) {
                $this->error('您还没有开通客户平台，无法完成本次操作', U('projectCustomer', array('id' => $where['customer_id'])), 2);
            }

            // 保存已选择的施工工序
            if (M('customer_platform')->where($where)->save($data)) {
                $this->success('添加成功', U('Project/projectCustomer'), 1);
            } else {
                $this->error('更新失败');
            }
        } else {
            // 列出所有施工工序
            $list = M('project')->where(array('admin_id' => fid()))->order('sort asc')->select();

            // 递归重组规则信息为多维数组
            $list = node_merge($list);

            // 当前客户已选施工工序
            $pro = M('customer_platform')->where(array('customer_id' => I('get.id', 0, 'intval')))->getField('project');
    ...
    public function addProjectInfo(){
        ...
        if (IS_POST) {
            // 验证必填字段是否符合规则
            $validate = array(
                array('pid', 'number','施工工序不能为空', 1),
                array('customer_id', 'number','客户ID只能输入整数', 1),
                array('title', 'require', '标题不能为空'),
                array('content', 'require', '施工内容不能为空')
            );
            D('platinfo')->setProperty("_validate",$validate);

            // 创建数据集
            $data = D('platinfo')->create();

            if (!$data) {
                $this->error(D('platinfo')->getError());
            }
            ...
            $url = './Uploads/Project/thumb/';  //定义图片的路径
            ...
            // 获取当前客户的施工工序
            $project = M('customer_platform')->where(array('customer_id' => I('get.id', 0, 'intval')))->getField('project');
    ...
    public function editorProjectInfo(){
        ...
        if (IS_POST) {
            ...
            // 获取缩略图处理并删除
            $thumb = M('platinfo')->where(array('id' => $data['id']))->getField('thumb');
            ...
            // 获取当前公司的施工工序
            $project = M('customer_platform')->where(array('customer_id' => I('get.cid', 0, 'intval')))->getField('project');
            $parr = unserialize($project);
    ...
    public function deleteProjectInfo(){
        ...
        // 删除
        if (M('platinfo')->where($where)->delete()) {
    ...
    public function viewProgress(){
        ...
        // 获取当前客户的所有施工详情
        $reslut = D('PlatinfoView')->where($where)->order('id desc')->select();
    ...
    public function pageCommon($where = array()){
        ...
        // 获取当前用户选择的客户ID
        $state = M('user_defined')->where(array('uid' => session('uid')))->getField('pstate');
        $state = explode(',', $state);
        $state = implode('|', $state);
        if(!empty($state)){
        ...
        $count  = D('CustomerView')->where($where)->count();  // 查询记录总数
        $Page   = new Page($count,30);          // 实例化分页类 传入总记录数
        $Page->setConfig('header','个客户');       // 定制分页样式

        // 跟踪状态
        $state = M('com_state')->field('id,state_id')->where(array('admin_id' => fid()))->order('sort')->select();
        session('state', $state);
        // 自定义显示字段
        $fields = M('user_defined')->field('project_field')->where(array('uid' => session('uid')))->find();
        session('project_field', $fields);
        ...
        $list = D('CustomerView')->where($where)->limit($Page->firstRow.','.$Page->listRows)->order('id desc')->select();
        ...
        // 符合查询条件的总单值
        $sum1  = D('CustomerView')->where($where)->Sum('OrdersValue');
        $dingjin  = D('CustomerView')->where($where)->Sum('Deposit');
        $space  = D('CustomerView')->where($where)->Sum('Space');
        $shejifei  = D('CustomerView')->where($where)->Sum('shejifei');
        $guanlifei  = D('CustomerView')->where($where)->Sum('guanlifei');
        $qingfu  = D('CustomerView')->where($where)->Sum('qingfu');
        $zhucai  = D('CustomerView')->where($where)->Sum('zhucai');
        $once  = D('CustomerView')->where($where)->Sum('once');
        $twice  = D('CustomerView')->where($where)->Sum('twice');
        $tirth  = D('CustomerView')->where($where)->Sum('tirth');
        $others  = D('CustomerView')->where($where)->Sum('others');
    ...
    public function comment()
    {
        if (IS_AJAX) {
            ...
            $data = array(
                ...
                'customer_name'=>M('customer')->where(array('id'=>$customer_id))->getField('CName'),
            );
